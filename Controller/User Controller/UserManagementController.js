...
const RetrieveAllUsers = (req, res) => {
  const query = `SELECT userId, name, email, role, status FROM Users`;
...
const RetrieveUserById = (req, res) => {
  ...
  const query = `SELECT userId, name, email, role, status FROM Users WHERE userId = ?`;
...
// PUT /v1/users/:id → Update user details (name, email, role)
